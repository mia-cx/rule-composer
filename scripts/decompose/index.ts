...
export const runDecompose = async (cliInputPath?: string, outputPath?: string): Promise<void> => {
  ...
  if (cliInputPath) {
    ...
    if (info.isDirectory()) {
      ...
      if (found.length === 0) {
        p.log.error(`No known rule files found in ${cliInputPath}. Looked for: ${SINGLE_FILE_RULES.join(", ")}`)
        return
      }
  ...
  if (detectedTool) {
    ...
    if (replacements.length > 0) {
      ...
      for (const r of replacements) {
        p.log.message(`  ${color.dim(r.value)} → ${color.green(`{{${r.variable}}}`)} ${color.dim(`(${r.count}×`)}`)
      }
  ...
  for (let i = 0; i < ruleFiles.length; i++) {
    ...
    const displayPath = rule.directory ? `${rule.directory}/${prefix}${rule.name}${ext}` : `${prefix}${rule.name}${ext}`
    if (await exists(filePath)) {
