...
export const buildTree = (sources: DiscoveredSource[]): TreeNode[] => {
  ...
  for (const source of sources) {
    // Group rules by their parent directory
    const dirMap = new Map<string, RuleFile[]>();

    for (const rule of source.rules) {
      const parts = rule.path.split("/");
      // Use the parent directory name as grouping key
      const parentIdx = parts.length - 2;
      const parent = parentIdx >= 0 ? parts[parentIdx]! : "";
    ...
    for (const [_dir, rules] of dirMap) {
...
const renderNode = (
  ...
  const hint = node.hint ? color.dim(` — ${node.hint}`) : "";

  const childCount =
    node.isDirectory && node.children
      ? color.dim(` (${node.children.length})`)
      : "";
 ...
 * - Left: Collapse directory (or move to parent if on file)
 * - Right: Expand directory
 * - Space: Toggle selection (directory = toggle all children)
 * - a: Toggle all
 * - Enter: Confirm
 */
export const treeMultiSelect = async (
  opts: TreeMultiSelectOptions,
): Promise<RuleFile[] | symbol> => {
      ...
      render() {
        ...
        const lines = visible.map(({ node, depth }, i) => {
          const siblings =
            depth === 0
              ? tree
              : visible.filter((v) => v.depth === depth).map((v) => v.node);
