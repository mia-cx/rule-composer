...
These rules define conventions for AI coding agents working in this project. Rules are stored in `{{RULES_DIR}}` as `{{RULES_EXT}}` files.
...
Use consistent patterns across the codebase. {{TOOL_NAME}} should follow these conventions strictly.
...
Use `{{RULES_DIR}}` for project-specific rules and `{{SKILLS_DIR}}` for reusable workflows.
