...
describe("decomposeResponseSchema", () => {
  ...
  it("rejects invalid names and short descriptions", () => {
    ...
    expect(
      decomposeResponseSchema.safeParse([
        { name: "coding", description: "Hi", headings: ["Coding"] },
      ]).success,
    ).toBe(false);
  });
  ...
  it("rejects invalid directory values", () => {
    ...
    // Uppercase
    expect(
      decomposeResponseSchema.safeParse([{ ...base, directory: "Testing" }])
        .success,
    ).toBe(false);
    // Underscores
    expect(
      decomposeResponseSchema.safeParse([{ ...base, directory: "my_tests" }])
        .success,
    ).toBe(false);
    // Trailing slash
    expect(
