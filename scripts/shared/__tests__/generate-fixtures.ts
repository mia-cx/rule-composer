...
const main = async () => {
  ...
  console.log(
    `Split into ${splits.length} sections: ${splits.map((s) => s.name).join(", ")}`,
  );
  ...
  console.log(
    `Wrote ${decomposeFiles.length} files to decompose-expected/: ${decomposeFiles.join(", ")}`,
  );
  ...
  const cursorResult = compose(readBackRules, "cursor");
  await writeAsSingleFile(cursorResult.content, join(COMPOSE_DIR, "AGENTS.md"));
  console.log(
    `Wrote compose-expected/AGENTS.md (cursor, ${cursorResult.placeholderCount} placeholders resolved)`,
  );

  // 6. Compose for claude target
  const claudeResult = compose(readBackRules, "claude");
  await writeAsSingleFile(claudeResult.content, join(COMPOSE_DIR, "claude.md"));
  console.log(
    `Wrote compose-expected/claude.md (claude, ${claudeResult.placeholderCount} placeholders resolved)`,
  );
