...
132 tests across 10 files covering placeholder resolution, rule composition, markdown splitting, schema validation, tree data structures, filesystem scanning, variant generation, decompose helpers, heading reconstruction, and end-to-end integration.
...
|----------|-------|---------|-----------------|
| Pure logic | 55 | `resolvePlaceholders`, `compose`, `estimateTokens`, `splitByHeadings`, all Zod schemas, `parseHeadingMap` | No I/O. Fast. Deterministic. |
| Data structures | 9 | `buildTree`, `getSelectedRules` | Tree construction from sources, selection filtering |
...
| File I/O | 26 | `readRule`, `writeAsSingleFile`, `writeAsDirectory`, `detectTools`, `resolveAgentsRepo`, `generateVariants` | Real filesystem via temp dirs. |
...
- [formats.test.ts](testing/formats) — Tool registry, variable maps, placeholder resolution, file I/O (23 tests)
- [composer.test.ts](testing/composer) — Rule composition and token estimation (9 tests)
- [splitter.test.ts](testing/splitter) — Heading-based markdown splitting (9 tests)
- [schemas.test.ts](testing/schemas) — Zod schema validation for 4 schemas (24 tests)
- [tree-prompt.test.ts](testing/tree-prompt) — Tree building and selection extraction (9 tests)
