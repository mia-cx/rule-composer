...
- **Markdown** (`.md`, `.mdc`): **@eslint/markdown** with `markdown/recommended`. Use `language: "markdown/commonmark"` so files are parsed as markdown, not JS. Rules off: `markdown/no-missing-label-refs` (callouts and task lists are not link refs), `markdown/no-space-in-emphasis` (callouts like `> [!globs]` trigger false positives).
